...
for (const [name, over] of SCEN) {
  ...
  if (on && !behind) {
    ...
    // 横幅在「已记账、等你确认」状态下讲的是**过去那一笔**，
    // 而卡片讲的是**下一步**，两者方向本来就可能不同，不该比。
    if (!/你做了吗/.test(btxt) && line.includes('买入') !== btxt.includes('买入')) {
...
console.log('\n================ 源码里的写死相对词 ================\n');
{
  // 先把行尾的 \r 去掉再判。JS 的 . 不匹配 \r，`.*$` 在 CRLF 行上必然失配 ——
  // 于是整行注释一个都剥不掉，几十条注释被当成用户文案报出来，
  // 而真正的问题淹没在里面。Windows 上随手存一次文件就会踩到。
